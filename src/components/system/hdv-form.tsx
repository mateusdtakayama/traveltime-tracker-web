...
const HDRFormSchema = z.object({
  startTime: z.string().refine(val => val !== "", { message: "Horário inicial é obrigatório" }),
  endTime: z.string().refine(val => val !== "", { message: "Horário final é obrigatório" }),
  hourValue: z
    .string()
    .refine(val => parseFloat(val) > 0, { message: "Valor por hora deve ser maior que 0" }),
}).refine(data => {
...
export function HDRForm() {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<HDRFormInputs>({
  ...
  const onSubmit = (data: HDRFormInputs) => {
    ...
    const formattedHours = Math.floor(hours).toString().padStart(2, '0') + ':' +
      Math.floor((hours % 1) * 60).toString().padStart(2, '0');
    ...
    <div className="max-w-md w-full mx-auto rounded-none md:rounded-2xl p-4 md:p-8 shadow-input bg-white dark:bg-black">
      ...
      <form className="my-8" onSubmit={handleSubmit(onSubmit)}>
        <div className="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-2 mb-4">
          <LabelInputContainer>
            ...
            <Input id="startTime" {...register("startTime")} placeholder="08:00" type="time" />
            {errors.startTime && <p className="text-red-500 text-sm">{errors.startTime.message}</p>}
          </LabelInputContainer>
          <LabelInputContainer>
            <Label htmlFor="endTime">Horário final</Label>
            <Input id="endTime" {...register("endTime")} placeholder="17:00" type="time" />
            {errors.endTime && <p className="text-red-500 text-sm">{errors.endTime.message}</p>}
          </LabelInputContainer>
        </div>
        <LabelInputContainer className="mb-4">
          <Label htmlFor="hourValue">Valor da hora</Label>
          <Input id="hourValue" {...register("hourValue")} placeholder="30,00" type="number" step="0.01" />
          {errors.hourValue && <p className="text-red-500 text-sm">{errors.hourValue.message}</p>}
        </LabelInputContainer>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent>
          ...
          <ResultDisplay totalHours={totalHours} totalValue={totalValue} toast={toast} />
        </DialogContent>
      </Dialog>
    </div>
  );
    ...
    < div className="space-y-4" >
      <div className="flex justify-between items-center">
        ...
        <div className="flex justify-center items-center gap-2">
          ...
          <span className="font-bold">{totalHours} <h3></h3></span>
          <CopyToClipboard text={totalHours} >

            <Button variant="ghost" className="outline-none" onClick={() => {
              toast({
